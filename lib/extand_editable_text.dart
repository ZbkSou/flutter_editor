...
class ExtandEditableText extends StatefulWidget {
  ...
  ///手动修改内容之后回调接口
  final ChangeCallback? onChangeListen;
  ...
  final ValueNotifier<SpecialTextContentList>? specialTextContentDataController;
...
class ExtandEditableTextState extends State<ExtandEditableText>
  ...
  bool get supportSpecialText =>
      ...
      _textDirection == TextDirection.ltr&&
          widget.specialTextContentDataController!=null;
  ...
  void initState() {
    ...
    ///zzzzz
    widget.specialTextContentDataController?.addListener(_handleSpecialTextContentChanged );
  ...
  void _handleSpecialTextContentChanged() {
    if(widget.specialTextContentDataController!=null){
      widget.controller.value = widget.controller.value.copyWith(
        text:  widget.specialTextContentDataController!.value.getContent(),
        selection: const TextSelection.collapsed(offset: -1),
        composing: TextRange.empty,
      );

      // specialTextContent = widget.specialTextContentDataController!.value;
    }
  ...
  void dispose() {
    ...
    widget.specialTextContentDataController?.removeListener(_handleSpecialTextContentChanged);
  ...
  TextEditingValue _handleSpecialTextSpan(TextEditingValue value) {
    if (supportSpecialText) {
      ...
      Diff diff = Diff(0, '', '');
      diff = getDiff(_value.text, diffText, cursorPosition);


      //修改specialTextContent,使样式数据同步
      if (diff.deleted.length > 0) {
        widget.specialTextContentDataController!.value.deleteTextContent(diff.start, diff.deleted);
      }

      if(diff.inserted.length > 0){
        widget.specialTextContentDataController!.value.addTextContent(diff.start, diff.inserted);
      }
      if (widget.onChangeListen != null) {
        widget.onChangeListen!(diff, value.selection);
      }
...
// //zzzz待样式的文字内容
//   SpecialTextContentList specialTextContent = SpecialTextContentList([]);
...
// null if no promptRect should be shown.
  ...
  TextSpan _buildTextSpan() {
    ...
    if (_value.composing.isValid && !widget.readOnly) {
      ...
      final SpecialTextContentList beforeTextContentList =
      widget.specialTextContentDataController!.value.substring(0, _value.composing.start);
      final SpecialTextContentList afterTextContentList =
      widget.specialTextContentDataController!.value.substring(_value.composing.end);
      final String insideText = _value.composing.textInside(_value.text);

      if (supportSpecialText) {
        final TextSpan before = widget.specialTextSpanBuilder!.build(
            specialTextContentList: beforeTextContentList,
            textStyle: widget.style);
        final TextSpan after = widget.specialTextSpanBuilder!.build(
            specialTextContentList: afterTextContentList,
            textStyle: widget.style);
    ...
    if (supportSpecialText) {
      final TextSpan? specialTextSpan = widget.specialTextSpanBuilder?.build(
          specialTextContentList:  widget.specialTextContentDataController!.value, textStyle: widget.style);
